...
# SECURITY FIX: Removed public write access (PutObject, DeleteObject) from bucket policy
...
output "security_warnings" {
  value = "WARNING: This bucket has some remaining misconfigurations (no encryption, no versioning), but public write access is now blocked!"
}
