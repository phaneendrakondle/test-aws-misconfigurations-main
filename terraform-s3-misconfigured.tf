...
# SECURITY FIX: Public access block enabled (blocks public write access)
...
# SECURITY FIX: Private ACL (no public write access)
...
# SECURITY FIX: Bucket policy with no public write access (removed PutObject and DeleteObject)
...
output "security_warnings" {
  value = "SECURITY FIX APPLIED: Public write access blocked. Bucket still has some misconfigurations (no encryption, no versioning) for testing purposes."
}
