...
# This file had public write access enabled, which has now been secured
# Remaining misconfigurations (for educational purposes): no encryption, versioning disabled
...
# FIXED: Removed write permissions from bucket policy (PutObject, DeleteObject removed)
...
output "security_warnings" {
  value = "NOTICE: Public WRITE access has been blocked. Bucket still has some misconfigurations: no encryption, versioning disabled."
}
