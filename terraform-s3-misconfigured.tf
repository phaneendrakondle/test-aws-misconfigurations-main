...
# FIXED: Restricted bucket policy - read-only access, no write permissions
...
output "security_warnings" {
  value = "NOTICE: Public write access has been blocked. Bucket now has read-only public access with public access block enabled. Note: No encryption and versioning still disabled."
}
